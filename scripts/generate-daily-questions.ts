...
// Helper function to repair and parse potentially malformed JSON
function repairAndParseJSON(text) {
  try {
    ...
    let repairedText = text
      // Fix unquoted property names (most common error)
    ...
    try {
      console.log("Repaired JSON - attempting to parse again");
      return JSON.parse(repairedText);
    } catch (secondError) {
      console.log("JSON repair failed:", secondError);
...
// --- IMPROVED: Helper function using categories from database records ---
async function generateGroundedCandidateQuestions(
    ...
    // Add specific instructions to use these topics for grounding
...
`;
    ...
    // Updated prompt with specific grounding instruction
    const prompt = `
...
**Crucial Instructions:**
    ...
    Example: (category: football) 
- question: Which player has won the most World Cups?
- options: ["Pelé", "Diego Maradona", "Franz Beckenbauer", "Cafu"]
- correct answer: Pelé
- explanation: Pelé won three World Cups (1958, 1962, 1970), the most by any player.

    **Output Format:**
    Format the output strictly as a JSON array containing exactly ${totalCandidates} objects. Each object must have keys: "category", "question", "options", "correct_answer", "explanation".
    - category: Must be exactly "${categories.join(', ')}".
    - question: String, max 200 characters, specific and engaging.
    - options: Array of exactly 4 unique strings, including the correct answer.
    - correct_answer: String, must be one of the options provided and **verifiably factually correct**.
    - explanation: String, a single concise sentence explaining why the answer is correct.

    Example format for targetCategory="tennis": [{"category": "tennis", "question": "About the selected tennis topic...", "options": [...], "correct_answer": "...", "explanation": "Concise factual reason..."}]
    Do not include any text, explanations, markdown formatting, or comments before or after the JSON array. Ensure the JSON is valid.
    `;
    ...
    try {
        ...
        if (!cleanedText || !cleanedText.startsWith('[')) {
            console.error("AI response snippet:", cleanedText?.substring(0, 200));
            throw new Error(`AI response is not a valid JSON array.`);
        }

        // Use the repair function instead of direct JSON.parse
        const potentialQuestions: any[] = repairAndParseJSON(cleanedText);
        
        if (!Array.isArray(potentialQuestions)) {
             throw new Error(`Parsed AI response is not an array.`);
        }

        const validCategories: Category[] = ['football', 'basketball', 'tennis', 'olympics'];
        const validatedQuestions = potentialQuestions
          .filter(q =>
              q &&

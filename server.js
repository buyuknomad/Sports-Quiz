// Fixed server.js with improved game ending logic
...
io.on('connection', (socket) => {
  ...
  socket.on('gameOver', ({ gameId }) => {
    ...
    // Check if game is already ending
    ...
    if (allReady && game.startCountdown === null) {
      ...
      const countdownInterval = setInterval(() => {
        if (game.startCountdown > 0) {
          ...
          game.startCountdown = null;
          game.questionStartTime = Date.now();
          game.lastAnswerTime = Date.now(); // Reset answer time tracker
    ...
    if (game.answeredPlayers.size === game.players.length) {
      ...
      if (game.currentQuestion < 9) {
        ...
        setTimeout(() => {
          ...
          if (!activeGames.has(gameId) || game.isGameEnded) return;
